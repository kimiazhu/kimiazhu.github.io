...
我们有一个需求希望能在C代码中执行文件系统扫描操作，这是耗时操作，通过NDK实现能够使得效率上有相当大的提升。这个问题在[《从Android本地代码扫描SD卡说开去》](http://scriptogr.am/kimiazhu/post/about-scanning-sdcard-in-native-code)中做过些探讨。
